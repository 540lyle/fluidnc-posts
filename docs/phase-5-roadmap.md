...
- Keep unresolved redistribution questions tracked in [Upstream Strategy](upstream-strategy.md) and [LLM Legal And Upstream](../llm/legal-and-upstream.md); this roadmap does not treat Autodesk redistribution rights as settled.
- Use a table-table (`AC`, trunnion-style) machine family as the first validated 5-axis target.
...
## Slice 0 gate before new runtime capability slices

Before Slice A or any later runtime slice starts, Slice 0 should first lock down the most visible 3-axis contract gaps:
...
These are not a separate runtime slice. They are the first contract decisions Slice 0 must make explicit before the adapter surface expands.
...
- expand controller specs before adapter work so drilling-cycle support, maximum work-offset range, tool length compensation semantics, movement classes, feed-clamp semantics, and coolant warning expectations are explicit
- expand machine/profile specs before adapter work so coolant capabilities, retract style, park positions, enabled work-offset subsets, machine-usable TLC policy, feed limits, probing templates, and multi-axis capability flags are explicit
- define the embedded profile catalog model for the shipped `.cps`
- define the first validated machine profile for a table-table (`AC`, trunnion-style) machine family
...
Capability ownership in this slice should stay explicit:

- controller specs declare the maximum controller-supported surface, such as `G54` through `G59.3` availability and whether tool length compensation exists at all
- machine profiles may only narrow, disable, or default those controller-supported capabilities for a specific machine

Slice 0 is done when it leaves behind concrete artifacts:

- a capability matrix in `specs/` comparing the current rewrite, imported original, and target behavior
- controller specs that explicitly cover drilling cycles, maximum work-offset range, tool length compensation semantics, movement classes, and coolant warning expectations
- at least one validated machine profile under `profiles/machines/`
- a documented embedded-profile catalog flow referenced from the profile and adapter docs
...
- extend work offsets through the controller-supported `G54` to `G59.3` range
...
- add optional tool length compensation handling where the controller contract permits it, plus feed validation and smarter spindle warmup behavior as profile-governed features
...
This slice should be validated against a table-table (`AC`, trunnion-style) machine profile and new indexed fixture families.
...
- profile fields for coolant mapping, retract and park behavior, enabled work-offset subsets, machine-usable TLC policy, feed limits, probing templates, multi-axis kinematics, and tested-fixture metadata
...
- controller specs define controller truth such as drilling-cycle support, maximum work-offset range, tool length compensation semantics, supported multi-axis feed semantics, probing command model, and explicit unsupported capabilities
- machine profiles define machine truth such as coolant hardware, enabled work-offset subsets, whether controller-allowed TLC is usable, kinematics, indexing behavior, park positions, limits, and available probing templates
...
4. add indexed `3+2` support for a table-table (`AC`, trunnion-style) profile
