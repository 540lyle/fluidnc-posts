...
longDescription = "FluidNC-optimized milling post. Arc tolerance and planner-aware segment filtering tuned for FluidNC firmware. Based on Grbl post with safety enhancements by Lyle.";
...
minimumChordLength = spatial(0.1, MM); // reduced from 0.25 for FluidNC arc fidelity
...
properties = {
  ...
  optionalStop: {
    title      : "Optional stop",
    description: "Outputs M1 (optional stop) before each tool change. Useful for manual tool change machines to pause for operator.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  safeStartAllOperations: {
    ...
    description: "When enabled, outputs modal codes (G90, G17, WCS, coolant, spindle) as optional blocks at the start of every operation. Useful when stitching split files or restarting mid-job.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  toolAsName: {
    title      : "Tool as name",
    description: "Use tool description as identifier instead of tool number. Disable for FluidNC (standard GRBL uses numeric tool calls).",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useG95: {
    ...
    description: "Enable to output G95 feed-per-revolution instead of G94 feed-per-minute. Note: FluidNC does not support G95 - leave disabled.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useDPMFeeds: {
    title      : "Use DPM feeds",
    description: "Enable to use degrees-per-minute feedrate mode for multi-axis moves instead of inverse time. Only applies to multi-axis machine configurations.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useTiltedWorkplane: {
    ...
    description: "Enable to use G68.2 tilted workplane for 3+2 indexing operations. Requires multi-axis machine configuration.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useCoolant: {
    title      : "Use coolant",
    description: "Enable to allow coolant M-code output (M7/M8/M9). Disable to suppress all coolant commands.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  spindleWarmupDelay: {
    ...
    description: "Dwell time in seconds after spindle start to allow the spindle to reach speed before cutting. Set to 0 to disable.",
    group      : "fluidnc",
    type       : "number",
    value      : 3,
    scope      : "post"
  ...
  fluidncArcTolerance: {
    ...
    description: "Match to your FluidNC firmware arc_tolerance_mm setting. The post aligns its linearization tolerance to this value so arcs are not re-interpolated at a coarser resolution by the controller. Default 0.002 matches FluidNC default.",
    group      : "fluidnc",
    type       : "number",
    value      : 0.002,
    scope      : "post"
  ...
  fluidncJunctionDeviation: {
    ...
    description: "Reference only - document your firmware junction_deviation_mm here. Recommended: 0.01 for finishing, 0.05 for roughing. The post emits clean geometry and lets FluidNC planner handle cornering via this parameter.",
    group      : "fluidnc",
    type       : "number",
    value      : 0.01,
    scope      : "post"
  ...
  minimumSegmentLength: {
    ...
    description: "Linear moves shorter than this distance from the last output point are suppressed to prevent FluidNC planner buffer starvation from over-segmented toolpaths. Set to 0 to disable. Typical range: 0.05-0.1 mm.",
    group      : "fluidnc",
    type       : "number",
    value      : 0.05,
    scope      : "post"
...
// FLUIDNC: segment filter tracks the last actually-output position to bound accumulated error
var _fluidncLastOutputPos;
var _fluidncPendingEndpoint; // stores the last skipped segment endpoint + feed to flush before mode changes

/** FLUIDNC: flush any pending segment-filter endpoint so toolpath endpoints are always reached. */
function _fluidncFlushPending() {
  if (_fluidncPendingEndpoint) {
    var p = _fluidncPendingEndpoint;
    _fluidncPendingEndpoint = undefined;
    // Output the pending endpoint - xOutput/yOutput/zOutput will handle detecting if values actually changed
    writeBlock(gMotionModal.format(1), xOutput.format(p.x), yOutput.format(p.y), zOutput.format(p.z), getFeed(p.feed));
    _fluidncLastOutputPos = {x:p.x, y:p.y, z:p.z};
  }
}
...
function onOpen() {
  ...
  // FLUIDNC: align post tolerance with FluidNC firmware arc_tolerance_mm.
  // Must be set before any toolpath processing, including split file mode.
  tolerance = spatial(getProperty("fluidncArcTolerance"), MM);
  ...
  // CUSTOM: startup safety block.
  writeBlock("G90", unit == MM ? "G21" : "G20", "G17"); // absolute coordinates, correct unit mode, XY plane
  writeBlock("G54");                 // default work offset
  writeBlock("G53 G0 Z0");           // retract to machine Z home
...
function onSection() {
  _fluidncLastOutputPos = undefined; // reset segment filter on new section
  _fluidncPendingEndpoint = undefined; // clear any pending endpoint from previous section
  ...
  // CUSTOM: force critical modals at section start for safer restarts and stitched files.
  writeBlock("G90", "G94", unit == MM ? "G21" : "G20", "G17");
  ...
  if (splitHere) {
    ...
    // absolute coordinates and feed per min
    writeProgramStart();
    writeProgramHeader(); // include machine info, tool table, and FluidNC config in each sub-file

    // CUSTOM: split-file startup safety block.
    writeBlock("G54");                 // default work offset
    writeBlock("G53 G0 Z0");           // retract to machine Z home
  }
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  _fluidncFlushPending(); // flush any skipped endpoint before arc
  _fluidncLastOutputPos = undefined; // reset segment filter after arc
...
function onSectionEnd() {
  _fluidncFlushPending(); // ensure section endpoint is reached before leaving section
...
function writeStartBlocks(isRequired, code) {
  ...
  var useOptionalBlocks = getSetting("supportsOptionalBlocks", true);
...
function startSpindle(tool, insertToolCall) {
  if (tool.type != TOOL_PROBE) {
    ...
    // CUSTOM: spindle warmup dwell after spindle start.
    if (spindleSpeedIsRequired && getProperty("spindleWarmupDelay") > 0) {
      writeBlock(gFormat.format(4), "P" + secFormat.format(getProperty("spindleWarmupDelay")));
    }
  }
}
// <<<<< INCLUDED FROM include_files/startSpindle.cpi
...
function writeProgramHeader() {
  ...
  writeln("");
  writeComment("FluidNC config: arc_tol=" + getProperty("fluidncArcTolerance") + "mm" +
    " junc_dev=" + getProperty("fluidncJunctionDeviation") + "mm" +
    " min_seg=" + getProperty("minimumSegmentLength") + "mm");
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  var safeStartRequiresFallback = !isRequired && getProperty("safeStartAllOperations", false) &&
    !getSetting("supportsOptionalBlocks", true);

  // CUSTOM: force a machine-coordinate Z retract before initial positioning.
  // Force Z-axis to machine home (G53 Z0) before ANY initial movement.
  forceModals(gAbsIncModal, gMotionModal);
  writeBlock("G53", "G0", "Z0");   // <-- Adjust if your safe Z is something else
  state.retractedZ = true;         // Mark Z as safely retracted to satisfy post logic
  ...
  if (!isRequired && !safeStartRequiresFallback) { // simple positioning
    var modalCodes = formatWords(gAbsIncModal.format(90), gPlaneModal.format(17));
    forceXYZ();
...
function onRapid(_x, _y, _z) {
  _fluidncFlushPending(); // flush any skipped endpoint before rapid
  _fluidncLastOutputPos = undefined; // reset segment filter after rapid
...
function onLinear(_x, _y, _z, feed) {
  // FLUIDNC: suppress micro-moves to reduce planner starvation from over-segmented paths.
  if (pendingRadiusCompensation < 0 && getProperty("minimumSegmentLength") > 0) {
    var _minSeg = spatial(getProperty("minimumSegmentLength"), MM);
    if (!_fluidncLastOutputPos) {
      _fluidncLastOutputPos = getCurrentPosition();
    }
    var _dx = _x - _fluidncLastOutputPos.x;
    var _dy = _y - _fluidncLastOutputPos.y;
    var _dz = _z - _fluidncLastOutputPos.z;
    var _distSq = _dx * _dx + _dy * _dy + _dz * _dz;
    if (_distSq > 0 && _distSq < (_minSeg * _minSeg)) {
      _fluidncPendingEndpoint = {x:_x, y:_y, z:_z, feed:feed}; // save for flush at mode change
      return; // skip micro-segment to reduce planner load
    }
  }
  _fluidncPendingEndpoint = undefined; // segment was output, clear pending
  _fluidncLastOutputPos = {x:_x, y:_y, z:_z};
  // FLUIDNC: end segment filtering block.
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  _fluidncFlushPending(); // flush any skipped endpoint before 5D rapid
  _fluidncLastOutputPos = undefined; // reset segment filter after 5D rapid
  if (pendingRadiusCompensation >= 0) {
    error(localize("Radius compensation mode cannot be changed at rapid traversal."));
    return;
  }
